 ...
 */
final class StorageGetHeadPostOrDeleteHttpHandler<C extends StorageHttpHandlerContext> implements GetHeadPostOrDeleteHttpHandler<C>,
     ...
     * Type safe getter
     */
    static <C extends StorageHttpHandlerContext> StorageGetHeadPostOrDeleteHttpHandler<C> with(final int pathAfter) {
        if(pathAfter < 0) {
            throw new IllegalArgumentException("Invalid path after " + pathAfter + " < 0");
        }
        return new StorageGetHeadPostOrDeleteHttpHandler<>(pathAfter);
    }

    private StorageGetHeadPostOrDeleteHttpHandler(final int pathAfter) {
        super();
     ...
     */
    private void acceptAndListStorage(final StoragePath path,
                                      final Accept accept,
     ...
     */
    private void listStorage(final StoragePath path,
                             final HttpRequest request,
                             final HttpResponse response,
                             final C context) {
     ...
     */
    private void loadStorageValue(final StoragePath path,
                                  final Accept accept,
                                  final HttpRequest request,
                                  final HttpResponse response,
                                  final C context) {
    ...
    private StoragePath extractStoragePath(final HttpRequest request) {
        final UrlPath urlPath = request.url()
            .path();
        return StoragePath.parse(
            urlPath.pathAfter(this.pathAfter)
                .value()
        );
    }

    private final int pathAfter;
}
