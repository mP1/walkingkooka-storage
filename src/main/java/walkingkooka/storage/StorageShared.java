...
abstract class StorageShared<C extends StorageContext> implements Storage<C> {
    ...
    private void failIfParent(final StoragePath path) {
        if(false == this instanceof StorageShared2WrapperPrefixed) {
            if(path.isParent()) {
    ...
    @Override
    public final void mount(final StorageMountPoint<C> mountPoint,
                            final C context) {
    ...
    abstract void mount0(final StorageMountPoint<C> mountPoint,
                         final C context);
